...
Schedule.prototype.addListeners = function () {
  ...
  o('html').on('mouseup', function (ev) {
    self.isMousedown = false

    if (self.selecting) {
      ev.preventDefault()
      ev.stopPropagation()
      self.emit('select', self.select, ev)
    }
    else if (self.resizing) {
      self.el.removeClass('schedule-resize-y')
      self.resizing.emit('change')
    }
    else if (self.dragging) {
      self.el.removeClass('schedule-move')
      self.dragging.emit('change')
    }
...
Schedule.prototype.hasColumn = function (id) {
  var col = this.columns.filter(function (col) { return id == col.id })[0]
  return !!col
}
...
Schedule.prototype.getXYByEvent = function (ev) {
  ...
  var xy = {
    x: Math.floor((ev.pageX-offset.left-51) / w)
